...
# Check out DBnomics, the world's economic database
# at https://db.nomics.world/

# You can access the data through the `rdbnomics` R package, available on:
...
# Fetch one series of the dataset "Balance of Payments" (BOP) 
...
df <- rdb(
  provider_code = "IMF", 
  dataset_code  = "BOP", 
  mask = "A.FR+DE.BCA_BP6_EUR+IA_BP6_EUR"
  ) |> 
  filter(!is.na(value)) |> 
  group_by(INDICATOR) |> 
  as_tibble() |> 
...
# dataset page on the DBnomics website
...
df <- rdb(
  api_link = "https://api.db.nomics.world/v22/series/SECO/qna_e_csa/gdp-tbgs.defl.L.Q?observations=1"
  ) |> 
  filter(!is.na(value)) |> 
  as_tibble()
...
# Note that when you update your card, you have to copy this link
# again, because the link itself contains the `ids` of the series 
# in the cart.

df <- rdb(
  api_link = "https://api.db.nomics.world/v22/series?observations=1&series_ids=BOE/6008/RPMTDDC,BOE/6231/RPMTBVE"
  ) |> 
  filter(!is.na(value)) |> 
  as_tibble()
...
# https://db.nomics.world/
# you need to give a provider of a dataset before 
# specifying the correct dimensions.

# With the function `dbnomics::rdb_datasets()`,
# you can download the list of all available datasets
# for a provider.
...
# 2. Use the default R itnernet connection, i.e. the Internet Explorer
...
# permanently iwth the package option
# `rdbnomics.curl_config`
...
ggplot(data = df[!is.na(value)], mapping = aes(x = period, y = value, color = series_name)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 2) +
  dbnomics()

ggsave(filename = "11_AMECO_ZUTN_EA19-DNK_filtered.png", path = "figures/04_rdbnomics-tutorial/", height = 8, width = 12)
graphics.off()
